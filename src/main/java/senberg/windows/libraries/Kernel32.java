package senberg.windows.libraries;
...
public interface Kernel32 extends StdCallLibrary, WinNT, Wincon {
     ...
     *
     * @param hMem A handle to the local memory object. If the <tt>hMem</tt> parameter
    ...
    boolean GetModuleHandleEx(int flags, String name, HMODULE outModule);

    /**
     * The GetSystemTime function retrieves the current system date and time.
     * The system time is expressed in Coordinated Universal Time (UTC).
     *
     * @param lpSystemTime Pointer to a {@link WinBase.SYSTEMTIME} structure to receive the current
     ...
     * The system time is expressed in Coordinated Universal Time (UTC).
     *
     * @param lpSystemTime Pointer to a {@link WinBase.SYSTEMTIME} structure holding the new
     ...
     *
     * @param lpSystemTime A pointer to a {@link WinBase.SYSTEMTIME} structure to receive the current
     *                     local date and time.
     ...
     * processors.
     *
     * @param lpIdleTime   A pointer to a {@link WinBase.FILETIME} structure that
     *                     receives the amount of time that the system has been idle.
     * @param lpKernelTime A pointer to a {@link WinBase.FILETIME} structure that
     ...
     * @param lpUserTime   A pointer to a {@link WinBase.FILETIME} structure that
     *                     receives the amount of time that the system has spent
     ...
     * specified process.
     *
     * @param process Handle to the process. The handle must have the
     ...
     *
     * @param processId Process identifier of the process of interest. A value of zero
     ...
     * @param lpExitCode A pointer to a variable to receive the process termination
     *                   status.
     ...
     * @param uExitCode The exit code to be used by the process and threads terminated
     ...
     * thread.
     *
     * @param dwErrCode Last-error code for the thread.
     */
     ...
     *
     * @param lpRootPathName Pointer to a null-terminated string that specifies the root
     ...
     * @param lpBuffer     Pointer to a pointer that receives the allocated buffer in
     *                     which the null-terminated string that specifies the formatted
     *                     message is written.
     * @param nSize        This parameter specifies the minimum number of TCHARs to
     *                     allocate for an output buffer.
     * @param va_list      Pointer to an array of values that are used as insert values
     ...
     *                           limit to 32,767 wide characters, prepend "\\?\" to the path.
     *                           For more information, see Naming a File.
     * @param bFailIfExists      If this parameter is TRUE and the new file specified by
     ...
     *
     * @param lpExistingFileName The current name of the file or directory on the local
     *                           computer.
     *                           <p>
     *                           The name is limited to MAX_PATH characters. To extend this
     *                           limit to 32,767 wide characters, prepend "\\?\" to the path.
     *                           For more information, see Naming a File.
     * @param lpNewFileName      The new name for the file or directory. The new name must not
     ...
     *                           dwFlags.
     * @param lpNewFileName      The new name of the file or directory on the local computer.
     *                           <p>
     ...
     *                           is empty.
     * @param dwFlags            This parameter can be one or more of the following values.
     * @return true, if successful If the function succeeds, the return value is
     * nonzero.
     * <p>
     * If the function fails, the return value is zero. To get extended
     ...
     *
     * @param lpPathName           Pointer to a null-terminated string that specifies the path of
     *                             the directory to be created.
     * @param lpSecurityAttributes Pointer to a SECURITY_ATTRIBUTES structure. The
     ...
     * @param ExistingCompletionPort    A handle to an existing I/O completion port or NULL.
     * @param CompletionKey             The per-handle user-defined completion key that is included in
     *                                  every I/O completion packet for the specified file handle.
     * @param NumberOfConcurrentThreads The maximum number of threads that the operating system can
     ...
     * @param lpNumberOfBytes A pointer to a variable that receives the number of bytes
     *                        transferred during an I/O operation that has completed.
     * @param lpCompletionKey A pointer to a variable that receives the completion key value
     *                        associated with the file handle whose I/O operation has
     *                        completed.
     * @param lpOverlapped    A pointer to a variable that receives the address of the
     *                        OVERLAPPED structure that was specified when the completed I/O
     *                        operation was started.
     * @param dwMilliseconds  The number of milliseconds that the caller is willing to wait
     ...
     *
     * @param CompletionPort             A handle to an I/O completion port to which the I/O completion
     ...
     * @param dwCompletionKey            The value to be returned through the lpCompletionKey parameter
     *                                   of the GetQueuedCompletionStatus function.
     * @param lpOverlapped               The value to be returned through the lpOverlapped parameter of
     ...
     *
     * @param hHandle        A handle to the object. For a list of the object types whose
     ...
     *
     * @param nCount         The number of object handles in the array pointed to by
     *                       lpHandles. The maximum number of object handles is
     *                       MAXIMUM_WAIT_OBJECTS.
     * @param hHandle        An array of object handles. For a list of the object types
     ...
     * @param bWaitAll       If this parameter is TRUE, the function returns when the state
     ...
     *
     * @param hSourceProcessHandle Handle to the process with the handle to duplicate. The handle
     *                             must have the PROCESS_DUP_HANDLE access right.
     * @param hSourceHandle        Handle to duplicate. This is an open object handle that is
     *                             valid in the context of the source process.
     * @param hTargetProcessHandle Handle to the process that is to receive the duplicated
     *                             handle. The handle must have the PROCESS_DUP_HANDLE access
     *                             right.
     * @param lpTargetHandle       Pointer to a variable that receives the duplicate handle. This
     ...
     * @param dwDesiredAccess      Access requested for the new handle.
     * @param bInheritHandle       Indicates whether the handle is inheritable.
     * @param dwOptions            Optional actions.
     * @return If the function succeeds, the return value is nonzero. If the
     * function fails, the return value is zero. To get extended error
     * information, call GetLastError.
     */
     ...
     *
     * @param directory         A handle to the directory to be monitored. This directory must
     *                          be opened with the FILE_LIST_DIRECTORY access right.
     * @param info              A pointer to the DWORD-aligned formatted buffer in which the
     *                          read results are to be returned.
     * @param length            The size of the buffer that is pointed to by the lpBuffer
     *                          parameter, in bytes.
     * @param watchSubtree      If this parameter is TRUE, the function monitors the directory
     ...
     * @param notifyFilter      The filter criteria that the function checks to determine if
     *                          the wait operation has completed.
     * @param bytesReturned     For synchronous calls, this parameter receives the number of
     ...
     * @param overlapped        A pointer to an OVERLAPPED structure that supplies data to be
     *                          used during asynchronous operation. Otherwise, this value is
     *                          NULL. The Offset and OffsetHigh members of this structure are
     *                          not used.
     * @param completionRoutine A pointer to a completion routine to be called when the
     ...
     * @param lpdzShortPath A pointer to a buffer to receive the null-terminated short
     *                      form of the path that lpszLongPath specifies.
     * @param cchBuffer     The size of the buffer that lpszShortPath points to, in
     *                      TCHARs.
     ...
     * and global heap.
     *
     * @param uFlags Memory allocation attributes. The default is the LMEM_FIXED
     *               value.
     * @param uBytes Number of bytes to allocate. If this parameter is zero and the
     ...
     * @param lpNumberOfBytesWritten A pointer to the variable that receives the number of bytes
     ...
     *
     * @param lpEventAttributes A pointer to a SECURITY_ATTRIBUTES structure. If this
     *                          parameter is NULL, the handle cannot be inherited by child
     *                          processes.
     * @param bManualReset      If this parameter is TRUE, the function creates a manual-reset
     ...
     * @param bInitialState     If this parameter is TRUE, the initial state of the event
     *                          object is signaled; otherwise, it is nonsignaled.
     * @param lpName            The name of the event object. The name is limited to MAX_PATH
     ...
     *
     * @param hEvent A handle to the event object. The CreateEvent or OpenEvent
     ...
     *                          object.
     * @param lpAttributes      A pointer to a SECURITY_ATTRIBUTES structure that determines
     ...
     *                          mapping object.
     * @param flProtect         Specifies the page protection of the file mapping object. All
     *                          mapped views of the object must be compatible with this
     *                          protection.
     * @param dwMaximumSizeHigh The high-order DWORD of the maximum size of the file mapping
     *                          object.
     * @param dwMaximumSizeLow  The low-order DWORD of the maximum size of the file mapping
     *                          object.
     ...
     * process.
     *
     * @param hFileMappingObject   A handle to a file mapping object. The CreateFileMapping and
     *                             OpenFileMapping functions return this handle.
     * @param dwDesiredAccess      The type of access to a file mapping object, which determines
     *                             the protection of the pages.
     * @param dwFileOffsetHigh     A high-order DWORD of the file offset where the view begins.
     * @param dwFileOffsetLow      A low-order DWORD of the file offset where the view is to
     *                             begin.
     ...
     *
     * @param lpBaseAddress A pointer to the base address of the mapped view of a file
     ...
     *
     * @param buffer  A pointer to a buffer that receives the computer name or the
     *                cluster virtual server name. The buffer size should be large
     *                enough to contain MAX_COMPUTERNAME_LENGTH + 1 characters.
     * @param lpnSize On input, specifies the size of the buffer, in TCHARs. On
     ...
     *
     * @param nameType An enumeration value specifying the type of name to be
     ...
     *
     * @param dwDesiredAccess Access to the thread object. This access right is checked
     *                        against any security descriptor for the thread.
     * @param bInheritHandle  If this parameter is TRUE, the new process inherits the
     *                        handle. If the parameter is FALSE, the handle is not
     *                        inherited.
     * @param dwThreadId      Identifier of the thread to be opened.
     ...
     * @param lpProcessAttributes  A pointer to a SECURITY_ATTRIBUTES structure that determines
     ...
     * @param lpThreadAttributes   A pointer to a SECURITY_ATTRIBUTES structure that determines
     *                             whether the returned handle to the new thread object can be
     *                             inherited by child processes. If lpThreadAttributes is NULL,
     *                             the handle cannot be inherited.
     * @param bInheritHandles      If this parameter TRUE, each inheritable handle in the calling
     ...
     *                             handles.
     * @param dwCreationFlags      The flags that control the priority class and the creation of
     *                             the process.
     * @param lpEnvironment        A pointer to the environment block for the new process. If
     ...
     *
     * @param lpApplicationName    The name of the module to be executed.
     * @param lpCommandLine        The command line to be executed. The maximum length of this
     ...
     *
     * @param hProcess  Handle for the running process
     * @param dwFlags   0 - The name should use the Win32 path format.
     *                  1(WinNT.PROCESS_NAME_NATIVE) - The name should use the native system path format.
     * @param lpExeName pre-allocated character buffer for the returned path
     * @param lpdwSize  input: the size of the allocated buffer
     ...
     * for temporary files.
     *
     * @param nBufferLength Size of the string buffer identified by lpBuffer, in TCHARs.
     * @param buffer        Pointer to a string buffer that receives the null-terminated
     ...
     *
     * @param lpVersionInfo Pointer to an OSVERSIONINFO data structure that the function
     ...
     * of the operating system that is currently running.
     *
     * @param lpVersionInfo Pointer to an OSVERSIONINFOEX data structure that the function
     *                      fills with operating system version information.
     * @return If the function succeeds, the return value is a nonzero value. If
     * the function fails, the return value is zero. To get extended
     * error information, call GetLastError. The function fails if you
     ...
     * @param Wow64Process Pointer to a value that is set to TRUE if the process is
     ...
     *
     * @param buffer       a buffer which receives an array of
     *                     {@link WinNT.SYSTEM_LOGICAL_PROCESSOR_INFORMATION} structures.
     * @param returnLength on input, specifies the length of the buffer in bytes. On
     ...
     * and virtual memory.
     *
     * @param lpBuffer A pointer to a MEMORYSTATUSEX structure that receives
     ...
     *
     * @param hFile                A handle to the file that contains the information to be retrieved.
     * @param FileInformationClass A FILE_INFO_BY_HANDLE_CLASS enumeration value that specifies the type of
     *                             information to be retrieved.
     * @param lpFileInformation    A pointer to the buffer that receives the requested file information.
     ...
     *
     * @param hFile                A handle to the file for which to change information.
     ...
     * @param FileInformationClass A FILE_INFO_BY_HANDLE_CLASS enumeration value that specifies the type of information to be changed.
     ...
     * @param lpFileInformation    A pointer to the buffer that contains the information to change for the specified file
     ...
     *
     * @param hFile            A handle to the file or directory for which dates and times
     *                         are to be retrieved. The handle must have been created using
     *                         the CreateFile function with the GENERIC_READ access right.
     * @param lpCreationTime   A pointer to a FILETIME structure to receive the date and time
     *                         the file or directory was created. This parameter can be NULL
     *                         if the application does not require this information.
     * @param lpLastAccessTime A pointer to a FILETIME structure to receive the date and time
     ...
     *                         information.
     * @param lpLastWriteTime  A pointer to a FILETIME structure to receive the date and time
     ...
     *
     * @param hFile            A handle to the file or directory. The handle must have been
     *                         created using the CreateFile function with the
     *                         FILE_WRITE_ATTRIBUTES access right. For more information, see
     *                         File Security and Access Rights.
     * @param lpCreationTime   A pointer to a FILETIME structure that contains the new
     *                         creation date and time for the file or directory. This
     *                         parameter can be NULL if the application does not need to
     *                         change this information.
     * @param lpLastAccessTime A pointer to a FILETIME structure that contains the new last
     ...
     * @param lpLastWriteTime  A pointer to a FILETIME structure that contains the new last
     ...
     *
     * @param lpFileName       The name of the file whose attributes are to be set.
     *                         <p>
     *                         The name is limited to MAX_PATH characters. To extend this
     *                         limit to 32,767 wide characters, prepend "\\?\" to the path.
     * @param dwFileAttributes The file attributes to set for the file. This parameter can be
     ...
     *
     * @param nBufferLength Maximum size of the buffer pointed to by lpBuffer, in TCHARs.
     *                      This size does not include the terminating null character. If
     *                      this parameter is zero, lpBuffer is not used.
     * @param lpBuffer      Pointer to a buffer that receives a series of null-terminated
     ...
     *
     * @param lpRootPathName          The root directory of the disk for which information is to be
     ...
     * @param lpSectorsPerCluster     A variable that receives the number of sectors per cluster.
     * @param lpBytesPerSector        A variable that receives the number of bytes per sector.
     * @param lpNumberOfFreeClusters  A variable that receives the total number of free clusters on the
     ...
     * @param lpTotalNumberOfClusters A variable that receives the total number of clusters on the
     *                                disk that are available to the user who is associated with the
     *                                calling thread. If per-user disk quotas are in use, this value
     ...
     *
     * @param lpDirectoryName          A string that specifies a directory on a disk. If this parameter
     ...
     *                                 directory on a disk.
     * @param lpFreeBytesAvailable     A pointer to a variable that receives the total number of free
     *                                 bytes on a disk that are available to the user who is
     *                                 associated with the calling thread. This parameter can be
     *                                 NULL.
     * @param lpTotalNumberOfBytes     A pointer to a variable that receives the total number of
     *                                 bytes on a disk that are available to the user who is
     *                                 associated with the calling thread. This parameter can be
     *                                 NULL.
     * @param lpTotalNumberOfFreeBytes A pointer to a variable that receives the total number of free
     ...
     * of the pipe.
     *
     * @param hReadPipe        A pointer to a variable that receives the read handle for the
     *                         pipe.
     * @param hWritePipe       A pointer to a variable that receives the write handle for the
     *                         pipe.
     * @param lpPipeAttributes A pointer to a SECURITY_ATTRIBUTES structure that determines
     ...
     * @param dwMask  A mask that specifies the bit flags to be changed. Use the
     *                same constants shown in the description of dwFlags.
     * @param dwFlags Set of bit flags that specifies properties of the object
     *                handle.
     * @return If the function succeeds, the return value is nonzero. If the
     * function fails, the return value is zero. To get extended error
     * information, call GetLastError.
     ...
     *
     * @param lpFileName The name of the file or directory. Prepend \\?\ to the path
     ...
     * @param dwIoControlCode The control code for the operation. This value identifies the
     ...
     * @param lpInBuffer      A pointer to the input buffer that contains the data required
     ...
     * @param nInBufferSize   The size of the input buffer, in bytes.
     * @param lpOutBuffer     A pointer to the output buffer that is to receive the data
     *                        returned by the operation. The format of this data depends on
     *                        the value of the dwIoControlCode parameter. This parameter can
     *                        be NULL if dwIoControlCode specifies an operation that does
     *                        not return data.
     * @param nOutBufferSize  The size of the output buffer, in bytes.
     * @param lpBytesReturned A pointer to a variable that receives the size of the data
     ...
     * @param lpOverlapped    A pointer to an OVERLAPPED structure. If hDevice was opened
     ...
     * @param th32ProcessID The process identifier of the process to be included in the
     ...
     * snapshot.
     *
     * @param hSnapshot A handle to the snapshot returned from a previous call to the
     ...
     *
     * @param lpName  Pointer to a string containing the name of the environment
     *                variable to set.
     * @param lpValue Pointer to a string containing the value to set it to. if this
     ...
     *
     * @param lpName   The name of the environment variable.
     * @param lpBuffer A pointer to a buffer that receives the contents of the
     ...
     * @param nSize    The size of the buffer pointed to by the lpBuffer parameter,
     ...
     * initialization file.
     *
     * @param appName      The name of the section in the initialization file.
     * @param keyName      The name of the key whose value is to be retrieved. This value
     *                     is in the form of a string; the {@link #GetPrivateProfileInt}
     *                     function converts the string into an integer and returns the
     *                     integer.
     * @param defaultValue The default value to return if the key name cannot be found in
     *                     the initialization file.
     * @param fileName     The name of the initialization file. If this parameter does
     ...
     *
     * @param lpAppName        The name of the section containing the key name. If this
     ...
     * @param lpKeyName        The name of the key whose associated string is to be
     ...
     *                         parameter.
     * @param lpDefault        A default string. If the {@code lpKeyName} key cannot be found
     ...
     * @param nSize            The size of the buffer pointed to by the
     *                         {@code lpReturnedString} parameter, in characters.
     * @param lpFileName       The name of the initialization file. If this parameter does
     *                         not contain a full path to the file, the system searches for
     *                         the file in the Windows directory.
     ...
     * characters.
     *
     * @param lpAppName  The name of the section to which the string will be copied. If
     ...
     * @param lpKeyName  The name of the key to be associated with a string. If the key
     ...
     * @param lpString   A string to be written to the file. If this parameter is
     ...
     * @param lpReturnedString A buffer that receives the key name and value pairs associated
     ...
     * </p>
     *
     * @param lpszReturnBuffer A pointer to a buffer that receives the section names
     ...
     * @param nSize            size of the buffer pointed to by the {@code lpszReturnBuffer}
     *                         parameter, in characters.
     * @param lpFileName       The name of the initialization file. If this parameter is
     ...
     *
     * @param lpFileTime      [in] A pointer to a FILETIME structure containing the
     *                        UTC-based file time to be converted into a local file time.
     * @param lpLocalFileTime [out] A pointer to a FILETIME structure to receive the
     ...
     *
     * @param lpTimeZone      [in, optional] A pointer to a TIME_ZONE_INFORMATION structure
     *                        that specifies the time zone of interest. If lpTimeZone is
     *                        NULL, the function uses the currently active time zone.
     * @param lpUniversalTime [in] A pointer to a SYSTEMTIME structure that specifies the
     ...
     *
     * @param lpSystemTime [in] A pointer to a SYSTEMTIME structure that contains the
     *                     system time to be converted from UTC to file time format.
     * @param lpFileTime   [out] A pointer to a FILETIME structure to receive the
     ...
     *
     * @param lpFileTime   [in] A pointer to a FILETIME structure containing the file time to be converted to system (UTC) date and time format.
     *                     This value must be less than 0x8000000000000000. Otherwise, the function fails.
     * @param lpSystemTime A pointer to a SYSTEMTIME structure to receive the converted file time.
     ...
     *
     * @param hProcess            A handle to the process with memory that is being read. The
     *                            handle must have PROCESS_VM_READ access to the process.
     * @param lpBaseAddress       A pointer to the base address in the specified process from
     *                            which to read. <br>
     ...
     * @param lpNumberOfBytesRead A pointer to a variable that receives the number of bytes
     ...
     *
     * @param lpFileName     The directory or path, and the file name. The file name can include wildcard characters,
     ...
     * @param lpFindFileData A pointer to the buffer that receives the file data. The pointer type is determined by the level of
     ...
     *                          you can opt-in to remove the MAX_PATH character limitation without prepending "\\?\".
     *                          See the "Maximum Path Limitation" section of Naming Files, Paths, and Namespaces for details.
     * @param fInfoLevelId      The information level of the returned data. This parameter is one of the FINDEX_INFO_LEVELS enumeration values.
     * @param lpFindFileData    A pointer to the buffer that receives the file data. The pointer type is determined by the level of
     *                          information that is specified in the fInfoLevelId parameter.
     * @param fSearchOp         The type of filtering to perform that is different from wildcard matching. This parameter is one of
     *                          the FINDEX_SEARCH_OPS enumeration values.
     * @param lpSearchFilter    A pointer to the search criteria if the specified fSearchOp needs structured search information.
     ...
     * @param dwAdditionalFlags Specifies additional flags that control the search.
     ...
     * device.
     *
     * @param hFile [in] A handle to the communications device.<br>
     *              The
     ...
     * @param lpDCB [in, out] A pointer to a {@link WinBase.DCB} structure that
     ...
     *              {@link com.sun.jna.platform.win32.Kernel32#CreateFile(String, int, int, com.sun.jna.platform.win32.WinBase.SECURITY_ATTRIBUTES, int, int, com.sun.jna.platform.win32.WinNT.HANDLE)}
     *              function returns this handle.
     * @param lpDCB [in] A pointer to a {@link WinBase.DCB} structure that
     ...
     *                       {@link com.sun.jna.platform.win32.Kernel32#CreateFile(String, int, int, com.sun.jna.platform.win32.WinBase.SECURITY_ATTRIBUTES, int, int, com.sun.jna.platform.win32.WinNT.HANDLE)}
     *                       function returns this handle.
     * @param lpCommTimeouts [in] A pointer to a {@link WinBase.COMMTIMEOUTS} structure
     ...
     *
     * @param dwProcessId Specifies a process identifier.<br>
     *                    Use the GetCurrentProcessId function to retrieve the process
     *                    identifier for the current process.
     * @param pSessionId  Pointer to a variable that receives the identifier of the
     ...
     * </pre>
     *
     * @param lpFileName A string that specifies the file name of the module to load.
     ...
     * @param flags      The action to be taken when loading the module.<br>
     ...
     * @param name    The name of the resource.<br>
     ...
     * @param type    The resource type.<br>
     *                Alternately, rather than a pointer, this parameter can be
     *                MAKEINTRESOURCE(ID), where ID is the integer identifier of the
     ...
     *
     * @param hModule   A handle to the module whose executable file contains the
     *                  resource. <br>
     *                  If hModule is NULL, the system loads the resource from the
     *                  module that was used to create the current process.
     * @param hResource A handle to the resource to be loaded. <br>
     ...
     *
     * @param hResource A handle to the resource to be accessed. <br>
     ...
     * @param hModule   A handle to the module whose executable file contains the
     *                  resource.
     * @param hResource A handle to the resource. This handle must be created by using
     ...
     *
     * @param module A handle to the loaded library module. The LoadLibrary,
     ...
     *
     * @param hModule A handle to a module to be searched.<br>
     ...
     * @param proc    A pointer to the callback function to be called for each
     *                enumerated resource type.<br>
     *                For more information, see the EnumResTypeProc function.
     * @param lParam  An application-defined value passed to the callback function.
     ...
     * @param type    The type of the resource for which the name is being
     *                enumerated.<br>
     *                Alternately, rather than a pointer, this parameter can be
